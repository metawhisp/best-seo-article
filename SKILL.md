...
5. Build a source plan, then source and claim ledgers before drafting load-bearing factual sections. For a named comparison, run a parity check for every decision criterion: inspect the same capability, limitation, permission, plan boundary, or processing stage for each named option. Record a supported difference or an explicit `unknown`; never build a recommendation by comparing a fully described feature on one side with a partial description on the other. Treat a first-party assertion as publishable evidence only when its owner, scope, date, method, and disclosure permission are recorded.
...
10. Freeze the first complete evidence-led draft as `drafts/baseline.md`, then make a bounded improvement pass. Repair only the smallest material gap that changes the reader decision; do not expand into a different intent merely to make the article longer. Apply a final humanization pass only after factual repair, preserving facts, citations, qualifications, and required disclosures. Read [references/iteration-loop.md](references/iteration-loop.md) before this pass.
11. Build `research/quality-gate.json` before approval: reader job, direct-answer location, decision criteria, section-to-evidence coverage, information gain, contextual length evidence, visual/data decision, the evidence-bound `reader_advantage`, and the baseline-to-final iteration assessment. Record the readable comparison in `research/iteration-report.md`. Full work must preserve the baseline and report what improved, regressed, and remains next; it may not hide an unresolved P0/P1 regression behind a stylistic rewrite.
12. Run an independent claim verification pass, then editorial, SEO, YMYL when applicable, media, and technical checks against the exact final bytes. The human editorial rubric must test answer/intent, truth/boundaries, information gain, practical utility, clarity/voice, and journey/conversion; it is not replaceable by a word count or aggregate score. Record machine-verifiable verdicts in bound JSON review records; keep `reviews/editorial.md` as human-readable notes, not the approval itself.
13. Package for the real destination. Create a CMS draft or publish only within the recorded permission boundary.
14. After publication, verify the live page; later, use `measurement-v1` with checksummed source evidence, equal half-open windows, matching timezone and grain, and exact metric descriptors. Bind the receipt to the final package manifest, live verification to both package and receipt, the baseline to the package, and each snapshot to the package plus live record. Do not claim causality from a before/after snapshot.

Read [references/research-and-evidence.md](references/research-and-evidence.md) for source and claim rules, [references/methodology.md](references/methodology.md) for the decision method, [references/iteration-loop.md](references/iteration-loop.md) for baseline-to-final repair and humanization, and [references/editorial-contract.md](references/editorial-contract.md) for drafting and review.
